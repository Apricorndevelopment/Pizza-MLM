...
@section('container')
    <div class="container mx-auto px-3 sm:px-4">
        ...
        <div class="flex justify-between items-center mb-6">
            <div class="flex items-center">
                ...
                <div>
                    <h3 class="text-2xl font-bold text-slate-800">User Gallery</h3>
                    <p class="text-slate-500 text-sm">Manage images displayed in the user gallery</p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                <h4 class="font-bold text-slate-700">All Images</h4>
                <span class="bg-indigo-100 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full">{{ $galleries->count() }} Records</span>
            </div>
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-50 text-slate-500 text-xs uppercase tracking-wider border-b border-slate-100">
                            ...
                            <th class="p-3 font-semibold">Image Preview</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                            ...
                            <tr class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="p-3">
                                    <div class="h-16 w-32 rounded-lg overflow-hidden border border-slate-200 shadow-sm relative group">
                                        <img src="{{ asset($gallery->photo) }}" class="h-full w-full object-cover transition-transform group-hover:scale-105">
                                    </div>
                                </td>
                                <td class="p-3">
                                    <p class="font-bold text-slate-700">{{ $gallery->title ?? 'No Title' }}</p>
                                </td>
                                <td class="p-3 text-right flex items-center justify-end gap-2">
                                    <button onclick='openModal("edit", @json($gallery))' class="text-indigo-500 hover:text-indigo-700 p-2 rounded-full hover:bg-indigo-50" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <form action="{{ route('admin.user.gallery.destroy', $gallery->id) }}" method="POST" onsubmit="return confirm('Delete this image?');" class="inline">
                                        @csrf @method('DELETE')
                        ...
                        @empty
                            <tr><td colspan="4" class="px-6 py-12 text-center text-slate-400">No Images Found</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    {{-- MODAL --}}
    <div id="galleryModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-middle bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full relative z-10">
                ...
                <form id="galleryForm" method="POST" enctype="multipart/form-data" class="p-6">
                    ...
                    <div class="space-y-5">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Gallery Image</label>
                            {{-- NAME CHANGED TO PHOTO TO MATCH MODEL --}}
                            <input type="file" name="photo" id="photo" onchange="previewFile(this)" class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-slate-50">
    ...
    <script>
        ...
        function openModal(type, data = null) {
            ...
            if (type === 'create') {
                title.innerText = 'Add New Gallery Image';
                btnText.innerText = 'Upload Image';
                form.action = "{{ route('admin.user.gallery.store') }}";
                methodField.innerHTML = ''; 
                form.reset();
                photoInput.required = true;
                previewContainer.classList.add('hidden');
            } else {
                title.innerText = 'Edit Gallery Image';
                btnText.innerText = 'Update Image';
                // Assuming route pattern /admin/user-gallery/update/{id}
                form.action = `/admin/user-gallery/update/${data.id}`;
