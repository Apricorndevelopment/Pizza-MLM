...
@section('container')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {{-- Total Revenue --}}
        <div class="bg-white rounded-xl shadow-sm p-3 border border-gray-100 hover:shadow-md transition-shadow">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-3 mb-6">
        ...
        <div
            class="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-xl shadow-lg p-3 text-white relative overflow-hidden">
            <div class="relative z-10">
                ...
                <div class="flex gap-3">
                    <a href="{{ route('admin.orders.index') }}"
                        class="px-4 py-2 bg-white text-blue-700 rounded-lg text-sm font-bold shadow-sm hover:bg-blue-50 transition">View
                        Orders</a>
                    <a href="{{ route('admin.viewmember') }}"
                        class="px-4 py-2 bg-blue-500 text-white border border-blue-400 rounded-lg text-sm font-bold hover:bg-blue-600 transition">View
                        Users</a>
                </div>
            </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            ...
            <a href="{{ route('admin.orders.index') }}" class="text-sm font-medium text-blue-600 hover:text-blue-800">View
                All Orders &rarr;</a>
        </div>
                ...
                <tbody class="divide-y divide-gray-100">
                    @forelse($recentOrders as $order)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="py-3 px-6 font-bold text-slate-700">₹{{ number_format($order->total_amount, 2) }}
                            </td>
                            <td class="py-3 px-6 text-gray-500">{{ $order->created_at->format('d M, Y') }}</td>
                            <td class="py-3 px-6 text-right">
                                @if ($order->status == 'delivered')
                                    <span
                                        class="bg-emerald-100 text-emerald-700 text-xs font-bold px-2.5 py-1 rounded-full">Delivered</span>
                                @elseif($order->status == 'placed')
                                    <span class="bg-amber-100 text-amber-700 text-xs font-bold px-2.5 py-1 rounded-full">New
                                        Order</span>
                                @elseif($order->status == 'cancelled' || $order->status == 'rejected')
                                    <span
                                        class="bg-red-100 text-red-700 text-xs font-bold px-2.5 py-1 rounded-full">{{ ucfirst($order->status) }}</span>
                                @else
                                    <span
                                        class="bg-blue-100 text-blue-700 text-xs font-bold px-2.5 py-1 rounded-full">{{ ucfirst($order->status) }}</span>
                                @endif
                            </td>
                        </tr>
                    @empty
