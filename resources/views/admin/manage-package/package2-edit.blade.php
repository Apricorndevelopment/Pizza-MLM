    ...
    <div class="max-w-5xl mx-auto">
        ...
        <div class="mb-8 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Update details for <span class="font-semibold text-blue-600">{{ $product->product_name }}</span></p>
            </div>
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <form action="{{ route('admin.package2.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="p-6 md:p-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-8">
                    ...
                    <div class="lg:col-span-7 space-y-6">
                        <div>
                            ...
                            <input type="text" name="product_name" value="{{ old('product_name', $product->product_name) }}"
                                class="w-full px-4 py-3 rounded-lg border border-gray-300 bg-gray-50 text-gray-900 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm placeholder-gray-400" required>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"><i class="fas fa-star"></i></span>
                                <input type="number" name="pv" value="{{ old('pv', $product->pv) }}" 
                                class="w-full pl-10 pr-4 py-3 rounded-lg border border-gray-300 bg-gray-50 text-gray-900 focus:bg-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all shadow-sm" required>
                            </div>
                        </div>
                    ...
                    <div class="lg:col-span-5">
                        ...
                        @php
                            $hasImage = $product->product_image && file_exists(public_path($product->product_image));
                            $imageUrl = $hasImage ? asset($product->product_image) : '#';
                        @endphp

                        {{-- Drop Zone (Hidden if image exists) --}}
                        <div id="drop-zone" class="{{ $hasImage ? 'hidden' : '' }} bg-gray-50 rounded-xl border-2 border-dashed border-gray-300 p-6 flex flex-col items-center justify-center text-center hover:bg-blue-50 hover:border-blue-400 transition-all cursor-pointer group h-full min-h-[250px]">
                        ...
                        <div id="preview-area" class="{{ $hasImage ? '' : 'hidden' }} h-full">
                            <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 h-full flex flex-col items-center justify-center relative group">
                                <div class="w-40 h-40 rounded-lg overflow-hidden bg-gray-100 border border-gray-100 mb-4 shadow-inner">
                                    <img id="preview-img" src="{{ $imageUrl }}" alt="Product Image" class="w-full h-full object-cover">
                                </div>
                                ...
                                <button type="button" id="remove-btn" class="inline-flex items-center px-4 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors text-sm font-medium">
                ...
                <div class="bg-blue-50/50 rounded-xl p-6 border border-blue-100">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        ...
                        <div class="group">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">GST % <span class="text-red-500">*</span></label>
                            <div class="relative">
                                <input type="number" step="0.01" name="gst" value="{{ old('gst', $product->gst) }}" 
                                class="w-full pl-4 pr-8 py-2.5 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all shadow-sm font-semibold text-gray-700" required>
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 font-semibold">₹</span>
                                <input type="number" step="0.01" name="dp" value="{{ old('dp', $product->dp) }}" 
                                class="w-full pl-8 pr-4 py-2.5 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all shadow-sm font-semibold text-gray-700" required>
                            </div>
                        </div>
                        ...
                        <div class="group">
                            ...
                            <div class="relative">
                                <input type="number" step="0.01" name="percentage" value="{{ old('percentage', $product->percentage) }}" 
                                class="w-full pl-4 pr-8 py-2.5 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all shadow-sm font-semibold text-green-600" required>
                                <span class="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 font-semibold">%</span>
                            </div>
                        </div>
                    </div>
