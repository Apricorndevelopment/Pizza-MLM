        ...
        <div x-data="{ activeTab: 'audio' }" class="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden">
            ...
            <div class="flex border-b border-slate-200">
                <button @click="activeTab = 'audio'" 
                    :class="activeTab === 'audio' ? 'border-indigo-500 text-indigo-600 bg-indigo-50' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'"
                ...
                <button @click="activeTab = 'video'" 
                    :class="activeTab === 'video' ? 'border-indigo-500 text-indigo-600 bg-indigo-50' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50'"
                    class="w-1/2 py-4 px-6 text-center font-bold text-sm uppercase tracking-wide border-b-2 transition-all">
                            ...
                            @foreach($audios as $audio)
                                <div class="bg-slate-50 rounded-xl p-4 border border-slate-200 shadow-sm relative group">
                                    <div class="flex items-center gap-3 mb-3">
                                        ...
                                        <h4 class="font-bold text-slate-700 truncate w-full" title="{{ $audio->title }}">{{ $audio->title }}</h4>
                                    </div>
                                    ...
                                    <div class="flex justify-between items-center text-xs text-slate-400">
                                        ...
                                        <form action="{{ route('admin.media.destroy', $audio->id) }}" method="POST" onsubmit="return confirm('Delete this audio?');">
                                            @csrf @method('DELETE')
                            ...
                            @foreach($videos as $video)
                                <div class="bg-slate-50 rounded-xl p-4 border border-slate-200 shadow-sm relative">
                                    ...
                                    <div class="flex justify-between items-center text-xs text-slate-400">
                                        ...
                                        <form action="{{ route('admin.media.destroy', $video->id) }}" method="POST" onsubmit="return confirm('Delete this video?');">
                                            @csrf @method('DELETE')
                                            <button class="text-red-400 hover:text-red-600 transition-colors" title="Delete">
                                                <i class="fas fa-trash-alt"></i> Delete
                                            </button>
    ...
    <div id="mediaModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-modal="true">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-middle bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-lg sm:w-full relative z-10">
                ...
                <form action="{{ route('admin.media.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
                    @csrf
                    ...
                    <div>
                        ...
                        <div class="flex gap-4">
                            <label class="flex items-center gap-2 cursor-pointer border p-3 rounded-lg w-1/2 hover:bg-slate-50 has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
                                <input type="radio" name="type" value="audio" checked class="text-indigo-600 focus:ring-indigo-500">
                                <span class="font-bold text-slate-700"><i class="fas fa-music mr-1"></i> Audio</span>
                            </label>
                            <label class="flex items-center gap-2 cursor-pointer border p-3 rounded-lg w-1/2 hover:bg-slate-50 has-[:checked]:border-indigo-500 has-[:checked]:bg-indigo-50">
                                <input type="radio" name="type" value="video" class="text-indigo-600 focus:ring-indigo-500">
                    ...
                    <div>
                        ...
                        <input type="text" name="title" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Enter Media title" required>
                    </div>
                    ...
                    <div class="flex justify-end pt-2">
                        <button type="button" onclick="closeModal()" class="px-4 py-2 mr-3 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50">Cancel</button>
                        <button type="submit" class="px-6 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 shadow-md">Upload</button>
                    </div>
                </form>
            </div>
        </div>
