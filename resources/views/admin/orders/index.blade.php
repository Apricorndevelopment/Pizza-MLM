        ...
        <div class="space-y-4" id="ordersContainer">
            @foreach($orders as $order)
                ...
                <div class="order-card bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow duration-300"
                     data-search="#{{ $order->order_id }} {{ strtolower($order->status) }} {{ strtolower($order->user->name ?? '') }} {{ strtolower($order->user->email ?? '') }}">
                        ...
                        <div class="flex flex-col md:flex-row gap-6 md:items-center">
                            ...
                            <div class="w-full md:w-2/12">
                                ...
                                <div class="text-xs text-slate-500 font-medium flex items-center">
                                    <i class="mdi mdi-calendar-clock mr-1.5"></i> {{ $order->created_at->format('M d, Y h:i A') }}
                                </div>
                            </div>
                            ...
                            <div class="w-full md:w-5/12 flex flex-col sm:flex-row items-center md:justify-end gap-3 md:pl-6">
                                ...
                                @if($order->status === 'delivered' || $order->status === 'rejected')
                                    <div class="w-full sm:w-auto px-4 py-2 rounded-lg bg-slate-100 border border-slate-200 text-slate-500 text-xs font-bold uppercase cursor-not-allowed flex items-center justify-center gap-2">
                                        @if($order->status === 'delivered') <i class="mdi mdi-check-all text-green-500"></i> @else <i class="mdi mdi-close-circle text-red-500"></i> @endif
                                        {{ $order->status }}
                                    </div>
                                @else
                                    {{-- UPDATE STATUS FORM --}}
                                    <form id="statusForm-{{ $order->id }}" action="{{ route('admin.orders.updateStatus') }}" method="POST" class="w-full sm:w-auto flex-grow">
                                        ...
                                        <div class="relative group">
                                            ...
                                            {{-- UPDATED ONCHANGE EVENT TO HANDLE MODAL --}}
                                            <select name="status" 
                                        ...
                                        <tr class="group/row hover:bg-white transition-colors">
                                            ...
                                            <td class="py-3 border-b border-slate-100">
                                                <div class="flex items-center gap-1.5">
                                                    <i class="mdi mdi-store text-emerald-500"></i>
                                            ...
                                            <td class="py-3 border-b border-slate-100 text-center">
                                                <span class="inline-block px-2 py-0.5 rounded bg-white border border-slate-200 text-xs font-bold text-slate-600">
                                                    {{ $item->quantity }}
                                                </span>
                                            </td>
...
{{-- IMPORTANT: Logic added for Admin Rejection --}}
...
<script>
    ...
    // 3. STATUS LOGIC (Handling Rejection Modal)
