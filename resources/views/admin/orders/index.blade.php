...
@section('container')
            ...
            <div class="w-full md:w-auto relative group">
                ...
                <div class="flex gap-4">
                    <input type="text" id="adminSearchInput"
                        ...
                        placeholder="Search Order ID, Customer, or Status..." autocomplete="off">
                        ...
                        <div class="flex flex-col md:flex-row gap-6 md:items-center">
                            ...
                            <div class="w-full md:w-2/12">
                                <div class="flex items-center gap-2 mb-2">
                                    <span
                                        class="inline-flex items-center px-2.5 py-1 rounded-lg text-sm font-semibold bg-[#ECFDF5] text-[#065F46] border border-emerald-100/50">
                                        #{{ $order->order_id }}
                                    </span>
                                </div>
                            ...
                            <div class="w-full md:w-2/12 md:border-l md:border-slate-100 md:pl-6">
                                <p class="text-[10px] uppercase tracking-wider font-bold text-slate-400 mb-0.5">Total Amount
                                </p>
                                <p class="text-xl font-bold text-[#064E3B]">₹{{ number_format($order->total_amount, 2) }}
                                </p>
                            ...
                            <div
                                ...
                                @if ($order->status === 'delivered' || $order->status === 'rejected')
                                    <div
                                        ...
                                        <i
                                            class="mdi mdi-{{ $order->status === 'delivered' ? 'check-all text-green-500' : 'close-circle text-red-500' }}"></i>
                                        {{ $order->status }}
                                    </div>
                                @else
                                    <form id="statusForm-{{ $order->id }}"
                                        action="{{ route('admin.orders.updateStatus') }}" method="POST"
                                        ...
                                        <div class="relative group">
                                            ...
                                            <select name="status"
                                                ...
                                                class="appearance-none block w-full pl-9 pr-8 py-2 text-xs font-bold bg-white border border-slate-200 rounded-lg text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#059669] shadow-sm hover:border-slate-300 uppercase cursor-pointer">
                                                @if ($order->status === 'placed')
                                                    <option value="placed" selected disabled>Placed (Action Required)
                                                    </option>
                                                    <option value="accepted" class="text-blue-600 font-bold">Accept Order
                                                    </option>
                                ...
                                <button type="button" onclick="toggleDetails('details-{{ $order->id }}')"
                                    class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-[#ECFDF5] hover:bg-emerald-100 text-[#059669] text-xs font-bold rounded-lg border border-[#10B981] transition-colors duration-200 gap-1 whitespace-nowrap">
                                    <span>Details</span><i id="icon-details-{{ $order->id }}"
                                            ...
                                            <tr class="group/row hover:bg-white transition-colors">
                                                ...
                                                <td class="py-3 border-b border-slate-100">
                                                    <div class="flex items-center gap-1.5"><i
                                                            class="mdi mdi-store text-emerald-500"></i><span
                                                            class="text-slate-600 font-medium text-xs">{{ $item->vendor->name ?? 'Unknown' }}</span>
                                                    </div>
                                                </td>
                                                <td class="py-3 border-b border-slate-100 text-slate-600 font-medium">
                                                    ₹{{ number_format($item->price, 2) }}</td>
                                                <td class="py-3 border-b border-slate-100 text-center"><span
    ...
    <div id="rejectModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog"
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <form id="rejectForm" action="{{ route('admin.orders.updateStatus') }}" method="POST">
                    @csrf
                    ...
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        <button type="submit"
                            class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm">Confirm
                            Rejection</button>
    ...
    <script>
        ...
        function closeRejectModal() {
            ...
            if (currentSelectElement) currentSelectElement.value = 'placed';
        }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function getOrderHtml(order) {
                ...
                let statusHtml = '';
                if (order.status === 'delivered' || order.status === 'rejected') {
                    const icon = order.status === 'delivered' ? 'check-all text-green-500' :
                        'close-circle text-red-500';
                    statusHtml =
                        `<div class="w-full sm:w-auto px-4 py-2 rounded-lg bg-slate-100 border border-slate-200 text-slate-500 text-xs font-bold uppercase cursor-not-allowed flex items-center justify-center gap-2"><i class="mdi mdi-${icon}"></i> ${order.status}</div>`;
                } else {
                    let options = '';
                    if (order.status === 'placed') {
                        options =
                            `<option value="placed" selected disabled>Placed (Action Required)</option><option value="accepted" class="text-blue-600 font-bold">Accept Order</option><option value="rejected" class="text-red-600 font-bold">Reject Order</option>`;
                    } else if (order.status === 'accepted') {
                        options =
                            `<option value="accepted" selected disabled>Accepted (Processing)</option><option value="delivered" class="text-green-600 font-bold">Mark Delivered</option>`;
                    }
                ...
                <div class="order-card bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow duration-300 animate-fade-in">
                    ...
                    <div id="details-${order.id}" class="max-h-0 overflow-hidden transition-all duration-500 ease-in-out opacity-0 bg-slate-50/50"><div class="p-6 border-t border-slate-100"><h6 class="text-[11px] uppercase tracking-widest font-bold text-slate-400 mb-4 flex items-center gap-2"><i class="mdi mdi-basket-outline text-lg"></i> Order Items</h6><div class="overflow-x-auto"><table class="w-full text-left border-collapse"><thead><tr class="text-xs text-slate-400 border-b border-slate-200"><th class="pb-3 font-semibold pl-1">Product</th><th class="pb-3 font-semibold">Vendor</th><th class="pb-3 font-semibold">Price</th><th class="pb-3 font-semibold text-center">Qty</th><th class="pb-3 font-semibold text-right pr-1">Subtotal</th></tr></thead><tbody class="text-sm">${itemsHtml}</tbody></table></div></div></div>
                </div>`;
            }
            ...
            async function fetchData(url, replace = false) {
                ...
                if (replace) {
                    container.innerHTML = getSkeletonHtml(3); // Search: Replace with skeletons
                    searchIcon.classList.add('hidden');
                    searchLoader.classList.remove('hidden');
                    endOfData.classList.add('hidden');
                } else {
                    container.insertAdjacentHTML('beforeend', getSkeletonHtml(2)); // Scroll: Append Skeletons
                }
                ...
                try {
                    ...
                    if (data.data.length > 0) {
                        ...
                        html =
                            `<div class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200"><div class="h-20 w-20 rounded-full bg-[#ECFDF5] flex items-center justify-center mb-4"><i class="mdi mdi-magnify-remove-outline text-4xl text-[#059669]"></i></div><h3 class="text-lg font-bold text-slate-800">No Orders Found</h3></div>`;
                    }
                    ...
                    if (replace) {
                        ...
                        removeSkeletons(); // Remove skeletons before adding real data
            ...
            searchInput.addEventListener('keydown', function(e) {
                ...
                if (e.key === 'Enter') {
                    ...
                    fetchData(url, true); // true = replace existing data
                }
            });
