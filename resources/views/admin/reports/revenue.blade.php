...
<div class="min-h-screen bg-slate-50 font-sans">
    <div class="max-w-7xl mx-auto px-2 sm:px-3 lg:px-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
            ...
            {{-- 3. MLM Distribution (Added Auto Pool) --}}
            <div class="bg-white rounded-2xl p-3 shadow-sm border border-slate-200">
                <p class="text-xs font-bold text-slate-400 uppercase mb-2">Network Income Distributed</p>
                <h2 class="text-2xl font-black text-red-500">₹{{ number_format($totalDistributedToOrders + $totalRewards + $autopoolIncome, 2) }}</h2>
                <div class="mt-2 text-[10px] text-slate-500 font-bold bg-red-50 inline-block px-2 py-0.5 rounded">Incomes + Rewards + Auto Pool</div>
            </div>

            {{-- 4. Final Net Profit --}}
            <div class="bg-slate-900 rounded-2xl p-3 shadow-lg border border-slate-800">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-3">
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="p-3 space-y-4">
                    {{-- Loop through distribution types with Auto Pool Added --}}
                    @php
                        $items = [
                            ...
                            ['label' => 'Auto Pool Earnings', 'val' => $autopoolIncome, 'icon' => 'bi-lightning-charge', 'color' => 'emerald'],
                        ];
                    @endphp
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="p-3">
                    <div class="space-y-5">
                        ...
                        <div class="flex justify-between border-b border-dashed border-slate-200 pb-3">
                            {{-- Added Auto pool income here --}}
                            <span class="text-sm text-slate-500">(-) Total MLM Payouts</span>
                            <span class="font-bold text-red-500">₹{{ number_format($totalDistributedToOrders + $totalRewards + $autopoolIncome, 2) }}</span>
                        </div>
