        ...
        <div class="max-w-7xl mx-auto">
            ...
            @if (session('success'))
                <div class="mb-3 rounded-xl bg-emerald-50 border border-emerald-200 p-4 flex items-center shadow-sm">
                    ...
                    <button type="button" class="ml-auto text-emerald-500 hover:text-emerald-800"
            ...
            @if (session('error'))
                <div class="mb-3 rounded-xl bg-red-50 border border-red-200 p-4 flex items-center shadow-sm">
                    ...
                    <button type="button" class="ml-auto text-red-500 hover:text-red-800"
                        onclick="this.parentElement.remove()"><i class="fas fa-times"></i></button>
                </div>
            @endif
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden mb-8">
                <div
                    class="px-3.5 py-3.5 border-b border-slate-100 bg-amber-50/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    ...
                    <div class="flex items-center gap-2 sm:gap-3">
                        {{-- Total Amount Badge --}}
                        <span
                            class="px-3 py-1 rounded-lg bg-white border border-slate-200 text-slate-600 text-xs font-bold shadow-sm flex items-center gap-1.5">
                            <i class="fas fa-coins text-amber-400"></i> Total:
                            <span class="text-amber-600 text-sm">₹{{ number_format($withdrawals->sum('amount'), 2) }}</span>
                        </span>

                        {{-- Pending Count Badge --}}
                        <span
                            class="px-2.5 py-1 bg-amber-100 text-amber-700 rounded-md text-xs font-bold border border-amber-200 shadow-sm">
                                ...
                                <tr class="hover:bg-slate-50 transition">
                                    <td class="p-3">
                                        <p class="font-bold text-slate-800 text-sm">
                                            {{ $req->vendor->company_name ?? 'N/A' }}</p>
                                        <p class="text-xs text-slate-500">{{ $req->user->name }} ({{ $req->user_ulid }})</p>
                                    </td>
                                    <td class="p-3">
                                        <span
                                            class="px-2 py-1 bg-slate-100 border border-slate-200 rounded text-[10px] font-bold text-slate-600 uppercase mb-1 inline-block">
                                            {{ $req->payment_method }}
                                        </span>
                                        @if ($req->payment_method == 'bank')
                                            <p class="text-xs text-slate-600 font-mono">{{ $req->user->account_no }} <br>
                                                IFSC: {{ $req->user->ifsc_code }}</p>
                                        @else
                                            <p class="text-xs text-slate-600 font-mono">{{ $req->user->upi_id }}</p>
                                        @endif
                                    </td>
                                    <td class="p-3 text-right font-bold text-slate-700">
                                        ₹{{ number_format($req->total_amount, 2) }}</td>
                                    <td class="p-3 text-right font-semibold text-red-500">-
                                        ₹{{ number_format($req->vendor_charge, 2) }}</td>
                                    <td class="p-3 text-right font-black text-emerald-600 text-lg">
                                        ₹{{ number_format($req->credited_amount, 2) }}</td>
                                    <td class="p-3 text-center">
                                        <div class="flex items-center justify-center gap-2">
                                            <form action="{{ route('admin.vendor.withdraw.approve', $req->id) }}"
                                                method="POST"
                                                onsubmit="return confirm('Are you sure you want to approve this withdrawal?')">
                                                @csrf
                                                <button type="submit"
                                                    class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 hover:bg-emerald-600 hover:text-white transition flex items-center justify-center"
                                                    title="Approve">
                                                    <i class="fas fa-check"></i>
                                                </button>
                                            </form>
                                            <form action="{{ route('admin.vendor.withdraw.reject', $req->id) }}"
                                                method="POST"
                                                onsubmit="return confirm('Are you sure you want to reject and refund this amount?')">
                                                @csrf
                                                <button type="submit"
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse ($allWithdrawls as $req)
                                <tr class="hover:bg-slate-50 transition">
                                    <td class="p-3 text-sm text-slate-500">{{ $req->created_at->format('d M Y, h:i A') }}
                                    </td>
                                    <td class="p-3">
                                        <p class="font-bold text-slate-800 text-sm">
                                            {{ $req->user->vendor->company_name ?? 'N/A' }}</p>
                                    ...
                                    <td class="p-3 text-center">
                                        @if ($req->status == 'approved')
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-50 text-emerald-600 border border-emerald-200"><i
                                                    class="fas fa-check-circle"></i> Approved</span>
                                        @else
                                            <span
                                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-red-50 text-red-600 border border-red-200"><i
