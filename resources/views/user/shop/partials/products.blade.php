                        ...
                        <div class="h-100 max-w-sm rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 ease-in-out bg-white border border-transparent hover:border-[#DBF5EC] group flex flex-col {{ $isOutOfStock ? 'opacity-80' : '' }}">
                            ...
                            <div class="p-2 md:p-3 flex flex-col justify-between flex-grow">
                                ...
                                <div class="product-action w-full mt-auto" id="action-admin-{{ $prod->id }}">
                                    ...
                                    @else
                                        <button onclick="addToCart({{ $prod->id }}, '{{ addslashes($prod->product_name) }}', {{ $prod->dp }}, 'admin', {{ $prod->max_coupon_usage ?? 0 }})"
    ...
    <div class="mb-5">
        ...
        <div class="row g-2 g-md-3">
            @foreach ($vendorProducts as $prod)
                ...
                @endphp
                <div class="col-6 col-md-4 col-lg-3 mb-3">
                    <div class="h-100 max-w-sm rounded-xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 ease-in-out bg-white border border-transparent hover:border-red-100 group flex flex-col {{ $isOutOfStock ? 'opacity-80' : '' }}">
                        ...
                        <div class="p-2 md:p-3 flex flex-col justify-between flex-grow">
                            ...
                            <div class="product-action w-full mt-auto" id="action-vendor-{{ $prod->id }}">
                                ...
                                @else
                                    <button onclick="addToCart({{ $prod->id }}, '{{ addslashes($prod->product_name) }}', {{ $prod->dp }}, 'vendor', {{ $prod->max_coupon_usage ?? 0 }})"
