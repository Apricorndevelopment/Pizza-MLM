...
@section('container')
    <div class="min-h-screen bg-slate-50/50 py-6 sm:py-10 font-sans">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if ($errors->any())
                <div class="mb-6 bg-red-50 border border-red-200 rounded-xl p-4 flex gap-3 animate-fade-in-up">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 mb-10">
                ...
                <div class="lg:col-span-5 order-1">
                    <div
                        class="bg-white rounded-2xl sm:rounded-3xl shadow-lg sm:shadow-xl overflow-hidden border border-slate-100 h-full flex flex-col relative group">

                        {{-- Decorative Gradient --}}
                        <div class="absolute top-0 inset-x-0 h-1.5 bg-gradient-to-r from-blue-600 to-indigo-600"></div>

                        <div class="p-6 sm:p-8 flex-1 flex flex-col items-center justify-center text-center">
                            <div class="mb-5 sm:mb-6">
                                <span
                                    class="inline-flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-indigo-50 text-indigo-600 mb-3 sm:mb-4">
                            ...
                            {{-- QR Code Container --}}
                            <div
                                class="relative bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border-2 border-slate-100 shadow-sm mb-6 group-hover:border-indigo-100 transition-colors duration-300 max-w-[260px] w-full mx-auto">
                                @if ($admin->upi_qr)
                                    <img src="{{ asset('storage/' . $admin->upi_qr) }}" alt="Admin QR"
                                        class="w-full h-auto object-contain rounded-lg">
                                @else
                                    <div
                                        class="aspect-square w-full bg-slate-50 flex flex-col items-center justify-center rounded-lg">
                                ...
                                {{-- Scan markers --}}
                                <div
                                    class="absolute top-0 left-0 w-5 h-5 sm:w-6 sm:h-6 border-t-4 border-l-4 border-indigo-600 rounded-tl-lg -mt-1 -ml-1">
                                </div>
                                <div
                                    class="absolute top-0 right-0 w-5 h-5 sm:w-6 sm:h-6 border-t-4 border-r-4 border-indigo-600 rounded-tr-lg -mt-1 -mr-1">
                                </div>
                                <div
                                    class="absolute bottom-0 left-0 w-5 h-5 sm:w-6 sm:h-6 border-b-4 border-l-4 border-indigo-600 rounded-bl-lg -mb-1 -ml-1">
                                </div>
                                <div
                                    class="absolute bottom-0 right-0 w-5 h-5 sm:w-6 sm:h-6 border-b-4 border-r-4 border-indigo-600 rounded-br-lg -mb-1 -mr-1">
                                </div>
                            </div>
                            ...
                            <div class="w-full max-w-xs">
                                <label
                                    class="block text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Official
                                    UPI ID</label>
                                <div
                                    class="flex items-center bg-slate-50 border border-slate-200 rounded-xl p-2.5 sm:p-3 select-all hover:border-indigo-300 transition-colors cursor-text group/copy">
                                    <div class="flex-shrink-0 mr-2 sm:mr-3 text-indigo-500">
                                        <i class="fas fa-wallet text-sm sm:text-base"></i>
                                    </div>
                                    <div class="flex-1 font-mono font-semibold text-slate-700 text-sm sm:text-lg truncate">
                                        {{ $admin->upi_id }}
                                    </div>
                                    <div class="text-slate-300 group-hover/copy:text-slate-500">
                        ...
                        <div class="bg-slate-50 p-3 sm:p-4 text-center border-t border-slate-100">
                ...
                <div class="lg:col-span-7 order-2">
                    <div
                        ...
                        <div class="p-6 sm:p-8">
                            @if (session('success'))
                                <div
                                    class="mb-6 rounded-xl bg-emerald-50 border border-emerald-100 p-4 flex gap-3 animate-fade-in-up">
                                    <div class="flex-shrink-0">
                            ...
                            <form action="{{ route('user.funds.store') }}" method="POST" enctype="multipart/form-data"
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                                    {{-- Sender UPI --}}
                                    <div>
                                        <label class="block text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">Your UPI
                                            ID</label>
                                    ...
                                    {{-- Payment Method --}}
                                    <div>
                                        <label class="block text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">Payment
                                            App</label>
                                        <div class="relative">
                                            <select name="payment_method"
                                                ...
                                                <option value="PhonePe"
                                                    {{ old('payment_method') == 'PhonePe' ? 'selected' : '' }}>PhonePe
                                                </option>
                                                <option value="Google Pay"
                                                    {{ old('payment_method') == 'Google Pay' ? 'selected' : '' }}>Google Pay
                                                </option>
                                                <option value="Paytm"
                                                    {{ old('payment_method') == 'Paytm' ? 'selected' : '' }}>Paytm</option>
                                                <option value="Other"
                                                    {{ old('payment_method') == 'Other' ? 'selected' : '' }}>Other UPI App
                                                </option>
                                            </select>
                                            <div
                                ...
                                <div>
                                    <label class="block text-sm font-semibold text-slate-700 mb-1.5 sm:mb-2">
                                        Payment Screenshot <span
                                            class="text-slate-400 font-normal text-xs">(Optional)</span>
                                    </label>
                                    <label
                                        class="flex flex-col items-center justify-center w-full h-28 sm:h-32 px-4 transition bg-slate-50 border-2 border-slate-200 border-dashed rounded-xl appearance-none cursor-pointer hover:border-indigo-300 hover:bg-white focus:outline-none group active:scale-[0.99] transition-transform">

                                        <div class="flex flex-col items-center space-y-2 text-center">
                                            <div
                                                class="p-2 bg-white rounded-full shadow-sm group-hover:shadow-md transition-shadow">
                                                <i
                                                    class="fas fa-cloud-upload-alt text-indigo-400 text-xl sm:text-2xl group-hover:text-indigo-600 transition-colors"></i>
                                            </div>
                                            <span
                                                class="font-medium text-slate-600 text-xs sm:text-sm group-hover:text-indigo-600">Tap
                                                to Upload Screenshot</span>
                                        </div>

                                        {{-- File Name Display --}}
                                        <div id="file-name-display"
                                            class="mt-2 text-xs font-semibold text-indigo-700 bg-indigo-50 px-3 py-1 rounded-full hidden truncate max-w-[200px]">
                                        </div>
            ...
            <div class="bg-white rounded-2xl sm:rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
                <div
                    class="px-6 py-4 sm:px-8 sm:py-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                    <h3 class="font-bold text-slate-800 text-base sm:text-lg">Transaction History</h3>
                    <span
                        class="hidden sm:inline-block px-3 py-1 bg-white border border-slate-200 rounded-full text-xs font-semibold text-slate-500 shadow-sm">
                        Recent Activity
                    </span>
                </div>
                    ...
                    <table class="w-full text-left border-collapse min-w-[600px]">
                        <thead>
                            <tr class="bg-slate-50 text-slate-500 border-b border-slate-100">
                                ...
                                <th class="px-6 sm:px-8 py-3 sm:py-4 text-xs font-bold uppercase tracking-wider">
                                    Transaction ID</th>
                                <th class="px-6 sm:px-8 py-3 sm:py-4 text-xs font-bold uppercase tracking-wider">Amount
                                </th>
                                <th
                                    class="px-6 sm:px-8 py-3 sm:py-4 text-xs font-bold uppercase tracking-wider text-right">
                                    Status</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-100">
                            @forelse($history as $item)
                                <tr class="hover:bg-slate-50/80 transition-colors group">
                                    ...
                                    <td
                                        class="px-6 sm:px-8 py-3 sm:py-4 text-sm font-mono text-slate-500 whitespace-nowrap group-hover:text-indigo-600 transition-colors">
                                        {{ $item->transaction_id }}
                                    </td>
                                    <td
                                        class="px-6 sm:px-8 py-3 sm:py-4 text-sm font-bold text-slate-800 whitespace-nowrap">
                                    ...
                                    <td class="px-6 sm:px-8 py-3 sm:py-4 text-right whitespace-nowrap">
                                        ...
                                        @elseif($item->status == 'rejected')
                                            <div class="flex flex-col items-end">
                                                ...
                                                {{-- Display Admin Remark if Rejected --}}
                                                @if ($item->admin_remark)
                                                    <span class="text-[10px] text-red-500 mt-1 max-w-[150px] truncate"
                                                        title="{{ $item->admin_remark }}">
                                        ...
                                        @else
                                            <span
                                                ...
                                                <span
                                                    class="w-1.5 h-1.5 rounded-full bg-amber-500 animate-pulse mr-1.5"></span>
                                                Pending
                                            </span>
                                        @endif
                                    </td>
                                ...
                                <tr>
                                    <td colspan="4" class="px-6 sm:px-8 py-10 sm:py-12 text-center text-slate-400">
                                        <div class="flex flex-col items-center justify-center">
                                            <div
                                                class="w-12 h-12 sm:w-16 sm:h-16 bg-slate-50 rounded-full flex items-center justify-center mb-3">
