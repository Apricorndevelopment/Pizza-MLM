...
@section('container')
    ...
    <div class="min-h-screen bg-slate-50 py-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            ...
            <div class="w-full md:w-auto relative group">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <i class="mdi mdi-magnify text-slate-400 text-lg group-focus-within:text-emerald-600 transition-colors"></i>
                </div>
        ...
        @if ($orders->count() > 0)
            <div class="space-y-4" id="ordersContainer">
                @foreach ($orders as $order)
                    ...
                    <div class="order-card bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden hover:shadow-md transition-shadow duration-300 relative"
                            ...
                            <div class="flex flex-col md:flex-row gap-6 md:items-center">
                                ...
                                <div class="w-full md:w-2/12">
                                    <div class="flex items-center gap-2 mb-2">
                                        <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-sm font-bold bg-[#ECFDF5] text-[#065F46] border border-emerald-100/50">
                                            #{{ $order->order_id }}
                                        </span>
                                    </div>
                                ...
                                <div class="w-full md:w-5/12 flex flex-col sm:flex-row items-center md:justify-end gap-3 md:pl-6">
                                    ...
                                    @else
                                        {{-- UPDATE STATUS FORM --}}
                                    <form id="statusForm-{{ $order->id }}" action="{{ route('vendor.orders.updateStatus') }}" method="POST" class="w-full sm:w-auto flex-grow">
                                        @csrf
                                        ...
                                        <div class="relative group">
                                            ...
                                            <select name="status" id="statusSelect-{{ $order->id }}" onchange="handleStatusChange(this, '{{ $order->id }}')" class="appearance-none block w-full pl-9 pr-8 py-2 text-xs font-bold bg-white border border-slate-200 rounded-lg text-slate-700 focus:outline-none focus:ring-2 focus:ring-[#059669] focus:border-transparent cursor-pointer shadow-sm hover:border-slate-300 transition-colors uppercase">
                                    ...
                                    @if ($order->status == 'delivered')
                                        <button type="button" onclick="openInvoiceModal('{{ $order->id }}')" class="w-full lg:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 font-bold rounded-lg border border-indigo-100 transition-all duration-200">
                                    ...
                                    <button type="button" onclick="toggleDetails('details-{{ $order->id }}')"
                                        ...
                                        <span>Details</span>
                                        <i id="icon-details-{{ $order->id }}" class="mdi mdi-chevron-down transition-transform duration-300"></i>
                                    </button>
                                </div>
                            </div>
                        </div>
                                            ...
                                            @foreach ($order->items as $item)
                                                <tr class="group/row hover:bg-white transition-colors">
                                                    ...
                                                    <td class="py-3 border-b border-slate-100 text-center">
                                                        <span class="inline-block px-2 py-0.5 rounded bg-white border border-slate-200 text-xs font-bold text-slate-600">{{ $item->quantity }}</span>
                                                    </td>
                        ...
                        {{-- EXACT INVOICE DESIGN IMPLEMENTATION (From User's Code) --}}
                        <div id="invoice-data-{{ $order->id }}" class="hidden">
                            <div class="p-3 sm:p-6 bg-white" style="font-family: sans-serif; color: #333;">
                                ...
                                <div class="flex justify-between items-center mb-6 pb-6 border-b border-gray-200">
                                    <div class="flex items-center gap-3">
                                        {{-- Platform Logo --}}
                                        <div class="w-10 sm:w-14 h-10 sm:h-14 bg-white rounded-lg border border-gray-100 shadow-sm p-1 flex items-center justify-center overflow-hidden">
                                            <img src="{{ asset('images/smartsave.png') }}" alt="Logo" class="w-full h-full object-contain">
                                        </div>
                                        <div>
                                            <h1 class="text-xl sm:text-2xl font-black text-teal-800 tracking-tight m-0 leading-none uppercase">ZIDDI GROUP</h1>
                                            <p class="text-[10px] text-gray-500 font-bold uppercase tracking-widest mt-1">SmartSave24 Platform</p>
                                        </div>
                                    </div>
                                    <div class="text-right">
                                        <h2 class="text-2xl sm:text-3xl font-bold text-gray-200 uppercase tracking-widest m-0 leading-none">BILL</h2>
                                ...
                                <div class="flex justify-between mb-8 gap-8">
                                    ...
                                    <div class="w-1/2">
                                        <p class="text-[8px] sm:text-[10px] font-bold text-teal-600 bg-teal-50 inline-block px-2 py-0.5 rounded uppercase tracking-widest mb-3">Billed To</p>
                                        <p class="font-extrabold text-gray-900 text-sm sm:text-lg leading-tight">{{ $order->user->name ?? 'Guest' }}</p>
                                        <p class="text-xs sm:text-sm text-gray-600 mt-1">{{ $order->user->email ?? 'N/A' }}</p>
                                        <p class="text-xs sm:text-sm text-gray-600">{{ $order->user->phone ?? 'N/A' }}</p>
                                    ...
                                    <div class="w-1/2 text-right">
                                        <p class="text-[8px] sm:text-[10px] font-bold text-gray-500 bg-gray-100 inline-block px-2 py-0.5 rounded uppercase tracking-widest mb-3">Sold By</p>

                                        @if ($isVendorOrder)
                                            <p class="font-extrabold text-gray-900 text-sm sm:text-lg leading-tight">{{ $firstItem->vendor->company_name ?? 'Vendor Store' }}</p>
                                            <p class="text-xs sm:text-sm font-bold text-teal-600 mt-0.5">Owner: {{ $firstItem->vendor->user->name ?? 'Vendor' }}</p>
                                            <p class="text-xs sm:text-sm text-gray-600 mt-1 max-w-[250px] ml-auto leading-snug">
                                                ...
                                                {{ $firstItem->vendor->company_city ?? '' }} {{ $firstItem->vendor->company_state ?? '' }}
                                            </p>
                                            <p class="text-xs sm:text-sm text-gray-500 mt-1">{{ $firstItem->vendor->user->email ?? '' }}</p>
                                        @else
                                            <p class="font-extrabold text-gray-900 text-sm sm:text-lg leading-tight">Ziddi Group Official</p>
                                        ...
                                        <div class="mt-4">
                                            <p class="text-xs font-semibold sm:font-bold text-gray-800 bg-gray-50 inline-block px-3 py-1.5 rounded border border-gray-100">
                                ...
                                <table class="w-full mb-6" style="width: 100%; border-collapse: collapse;">
                                    <thead>
                                        <tr class="bg-gray-100 border-b-2 border-gray-200">
                                            <th class="text-left p-3 text-xs font-extrabold text-gray-600 uppercase">Item Description</th>
                                            <th class="text-center p-3 text-xs font-extrabold text-gray-600 uppercase">Qty</th>
                                            <th class="text-right p-3 text-xs font-extrabold text-gray-600 uppercase">Price</th>
                                            <th class="text-right p-3 text-xs font-extrabold text-gray-600 uppercase">Total</th>
                                        </tr>
                                    </thead>
                                    <tbody class="divide-y divide-gray-100">
                                        @foreach ($order->items as $item)
                                            <tr>
                                                <td class="py-3 px-3 text-sm font-bold text-gray-800">{{ $item->product_name }}</td>
                                                <td class="py-3 px-3 text-sm font-medium text-gray-600 text-center">{{ $item->quantity }}</td>
                                                <td class="py-3 px-3 text-sm font-medium text-gray-600 text-right">₹{{ number_format($item->price, 2) }}</td>
                                                <td class="py-3 px-3 text-sm font-black text-teal-700 text-right">₹{{ number_format($item->subtotal, 2) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                                ...
                                <div class="flex justify-end">
                                    <div class="w-1/2 bg-gray-50 rounded-xl p-2 sm:p-4 border border-gray-100">
                                        ...
                                        <div class="flex justify-between py-2 border-b border-gray-200/60">
                                            ...
                                            <span class="text-sm font-bold text-gray-800">₹{{ number_format($order->wallet1_deducted, 2) }}</span>
                                        </div>
                                        @if ($order->wallet2_deducted > 0)
                                            <div class="flex justify-between py-2 border-b border-gray-200/60">
                                                <span class="text-sm font-medium text-gray-600">Second Wallet Used</span>
                                                <span class="text-sm font-bold text-gray-800">₹{{ number_format($order->wallet2_deducted, 2) }}</span>
                                            </div>
                                        @endif
                                        @if ($order->coupons_used > 0)
                                            <div class="flex justify-between py-2 border-b border-gray-200/60">
                                                <span class="text-sm font-medium text-gray-600">Coupon Discount</span>
                                                <span class="text-sm font-bold text-orange-500">- ₹{{ number_format($order->coupons_used * 10) }}</span>
                                            </div>
                                        @endif

                                        <div class="flex justify-between py-3 mt-1">
                                            <span class="text-sm sm:text-base font-extrabold text-gray-900 uppercase">Total Paid</span>
                                            <span class="text-xl font-black text-teal-700">₹{{ number_format($order->wallet1_deducted + $order->wallet2_deducted, 2) }}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="mt-12 pt-8 border-t-2 border-dashed border-gray-200 text-center">
                                    <p class="text-base font-black text-gray-800">Thank you for shopping with Ziddi Group!</p>
                                    <p class="text-xs font-medium text-gray-500 mt-1">For support, please contact your seller or reach out to Ziddi Group Helpdesk.</p>
                                    <p class="text-[10px] text-gray-400 mt-4 uppercase tracking-widest">This is a computer generated invoice</p>
                                </div>
                            </div>
                        </div>
                ...
                <div id="invoiceModal" class="fixed inset-0 z-50 hidden" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                    <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity" onclick="closeInvoiceModal()"></div>
                    <div class="fixed inset-0 z-10 w-screen p-2 overflow-y-auto">
                        <div class="flex min-h-screen items-center justify-center text-center sm:p-2">
                            <div class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 w-full max-w-2xl">
                                <div class="bg-gray-50 px-4 py-3 sm:px-6 flex justify-between items-center border-b border-gray-200">
                                    ...
                                    <button type="button" onclick="closeInvoiceModal()" class="text-gray-400 hover:text-gray-500 focus:outline-none rounded-full p-1 hover:bg-gray-100 transition-colors">
                                ...
                                <div class="bg-white px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6 border-t border-gray-200 gap-2">
                                    <button type="button" onclick="downloadPDF()" class="inline-flex w-full justify-center rounded-xl bg-teal-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-teal-700 sm:ml-3 sm:w-auto gap-2 items-center transition-colors">
                                        <i class="bi bi-download"></i> Download PDF
                                    </button>
                                    <button type="button" onclick="closeInvoiceModal()" class="mt-3 inline-flex w-full justify-center rounded-xl bg-white px-5 py-2.5 text-sm font-bold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto transition-colors">
                                        Cancel
                                    </button>
                                </div>
                            </div>
            ...
            {{-- No Results Message --}}
            <div id="noResultsMessage" class="hidden flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
        ...
        @else
            ...
            <div class="flex flex-col items-center justify-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                ...
                <h3 class="text-lg font-bold text-slate-800">No Orders Found</h3>
                <p class="text-slate-500 text-sm mt-1 mb-6 text-center max-w-xs">There are no orders available at the moment.</p>
            </div>
        @endif
    </div>
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form id="rejectForm" action="{{ route('vendor.orders.updateStatus') }}" method="POST">
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500 mb-3">Please specify the reason for rejecting this order. This will be visible to the system.</p>
                                    <textarea name="reason" rows="3" required class="w-full border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500 sm:text-sm border p-2" placeholder="Type reason here (e.g., Out of Stock, Invalid Address)..."></textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                        ...
                        <button type="button" onclick="closeRejectModal()" class="w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
                            Cancel
                        </button>
                    </div>
                </form>
    ...
    <div id="deliveryOtpModal" class="fixed inset-0 z-[60] hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" onclick="closeOtpModal()"></div>
            <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 mb-3">Ask the customer for the 6-digit OTP shown in their 'My Orders' section to confirm delivery.</p>
                                <input type="text" id="modalOtpInput" required class="w-full text-center tracking-widest font-mono text-2xl border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-lg border p-3" placeholder="• • • • • •" maxlength="6">
                            </div>
                        </div>
                    </div>
                </div>
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    <button type="button" onclick="submitOtpDelivery()" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-green-600 text-base font-medium text-white hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 sm:ml-3 sm:w-auto sm:text-sm">
                        Verify & Deliver
                    </button>
